...
ここまでの原則を 1 つの例にまとめる。**説明用の縮約コードであり実在ファイルではない**。「public 面だけをテストする」「正常系・エッジ・異常系を押さえる」「real CPU tensor + seed 固定」「private は触らない」を 1 セットで示す。

### ソース (`exp/models/components/example.py`)
...
### テスト (`tests/models/components/test_example.py`)
...
```python
# ❌ private helper を直接テスト — _prefix を import している時点で規約違反。
...
`_safe_std` の「分散 0 を eps で下支えする」振る舞いは、上の `test_constant_input_does_not_divide_by_zero` が **public 面越しに既に担保している**。private を直接叩く必要はない。
