...
class _ImageEncoderCall(Protocol):
    """``ImageEncoder.__call__`` の型を入力 shape ごとに伝える Protocol。

    ``nn.Module.__call__`` は ``Any`` を返すため、forward の入出力対応 (単一 → 単一 / バッチ
    → バッチ / 系列 → 系列 / バッチ系列 → バッチ系列) を呼び出し側へ overload で伝える (runtime は
    nn.Module.__call__ が hooks 経由で forward に dispatch)。
    """
...
class ImageEncoder(nn.Module):
    ...
    # overload は Protocol でしか表現できず nn.Module.__call__ との override 互換
    # 判定に引っかかるため、この行だけ抑制する (意図的な型の絞り込み)。
    __call__: _ImageEncoderCall  # type: ignore[reportIncompatibleMethodOverride]
